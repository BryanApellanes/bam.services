...
namespace Bam.Net
{
    public class ServiceExe : ServiceBase
    {
        ...
        protected static void FileLog(Exception fatalEx)
        {
            lock (lastDitchLock)
            {
                string logPath = Path.Combine(RuntimeSettings.ProcessDataFolder, $"{serviceName}.fatal.log");
        ...
        public static void OnStartInstall()
        {
            if (StartInstall != null)
            {
                try
                {
                    Message.PrintLine(string.Format("INFO:: Calling Pre-Install handlers of {0}...", serviceName));
                ...
                catch (Exception ex)
                {
                    ...
                    Message.PrintLine(string.Format(messageFormat, serviceName));
        ...
        public static void OnEndInstall()
        {
            if (EndInstall != null)
            {
                try
                {
                    Message.PrintLine(string.Format("INFO:: Calling Post-Install handlers of {0}...", serviceName));
        ...
        public static void OnStartUninstall()
        {
            if (StartUninstall != null)
            {
                try
                {
                    Message.PrintLine(string.Format("INFO:: Calling Pre-Uninstall handlers of {0}...", serviceName));
        ...
        public static void OnEndUninstall()
        {
            if (EndUninstall != null)
            {
                try
                {
                    Message.PrintLine(string.Format("INFO:: Calling Post-Uninstall handlers of {0}...", serviceName));
        ...
        {
            string? userName = string.IsNullOrEmpty(credentialKey) ? null : Configuration.DefaultConfiguration.GetAppSetting(credentialKey);
            string? password = string.IsNullOrEmpty(credentialKey) ? null : Configuration.DefaultConfiguration.GetAppSetting(credentialKey + "Password");
        ...
        public static void Install(string serviceName, string displayName, string description, bool allowDesktopInteraction, string serviceAccountName, string serviceAccountPassword)
        {
            ...
            try
            {
                ...
                Message.PrintLine("INFO:: Creating service from " + cur.Location);
                ManagementClass win32Service = new ManagementClass(@"\\.\root\cimv2:Win32_Service");

                Message.PrintLine("INFO:: ServiceName={0},DisplayName={1},Description={2}", serviceName, displayName, description);
                ...
                {
                    Message.PrintLine("INFO:: Service was created successfully");
                }
                else
                {
                    Message.PrintLine("WARN:: Return code was: {0}", ret.ToString());
                    withIssues = true;
                }

                if (!string.IsNullOrEmpty(description))
                {
                    Message.PrintLine("INFO:: Setting description for '{0}' to '{1}'", displayName, description);
                ...
                if (!withIssues)
                {
                    Message.PrintLine("INFO:: Operation complete");
                }
                else
                {
                    Message.PrintLine("WARN:: Operation completed with some issues");
                }
            }
            catch (Exception ex)
            {
                Message.PrintLine("ERROR:: Unable to create service: " + ex.Message);
            }
        ...
        public static void Uninstall(string serviceName)
        {
            ...
            try
            {
                Message.PrintLine("INFO:: Un-installing service \"{0}\"", serviceName);
                try
                {
                    ServiceController sc = new ServiceController(serviceName);
                    if (sc.Status == ServiceControllerStatus.Running)
                    {
                        Message.PrintLine("INFO:: Stopping service '" + serviceName + "'...");
                        sc.Stop();
                ...
                {
                    Message.PrintLine("WARN:: Request to stop service caused an error: " + ex.Message);
                }
                ...
                if (ret == 0)
                {
                    Message.PrintLine("INFO:: Uninstall complete");
                }
                else
                {
                    Message.PrintLine("WARN:: Uninstall returned code " + ret.ToString());
                }
            }
            catch (Exception ex)
            {
                Message.PrintLine("ERROR:: Unable to uninstall service: {0}", ex.Message);
            }
        ...
        public static void Restart(string serviceName)
        {
            ...
            {
                ServiceController sc = new ServiceController(serviceName);
                Message.PrintLine("INFO:: Stopping service \"{0}\"", serviceName);
                sc.Stop();
                sc.WaitForStatus(ServiceControllerStatus.Stopped, new TimeSpan(0, 0, 10));
                Message.PrintLine("INFO:: Starting service \"{0}\"", serviceName);
                sc.Start();
            ...
            {
                Message.PrintLine("ERROR:: An error occurred restarting the service: {0}",
                    ex.Message);
            }
        }
        ...
        public static void Start(string serviceName)
        {
            ...
            {
                Message.PrintLine("ERROR:: An error occurred starting the service: {0}", ex.Message);
            }
        }
        ...
        public static void Kill(string serviceName)
        {
            ...
            {
                Message.PrintLine("ERROR:: An error occurred stopping the service: {0}", ex.Message);
            }
        }
        ...
        public static void DeleteLog(bool force, string logName)
        {
            if (force)
            {
                Message.PrintLine("INFO: Deleting log...");
                EventLog.Delete(logName);
                Message.PrintLine("INFO: Operation complete");
                return;
            }

            Message.PrintLine("You are about to delete the Windows event log {0}", logName);
            Message.PrintLine("Do you want to continue? (y/N)");
        ...
        {
            logger = (WindowsLogger)Log.CreateLogger("Windows");
            logger.ApplicationName = svcName;
